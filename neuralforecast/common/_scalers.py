...
# %% auto 0
__all__ = ['masked_median', 'masked_mean', 'masked_absmean', 'minmax_statistics', 'minmax1_statistics', 'std_statistics', 'robust_statistics',
...
def masked_meanabs(x, mask, dim=-1, keepdim=True):
    """Masked Absolute Mean
    ...
    **Returns:**<br>
    ...
    """
    x_nan = x.float().masked_fill(mask < 1, float("nan"))
    x_mean = x_nan.abs().x_nan.nanmean(dim=dim, keepdim=keepdim)
    x_mean = torch.nan_to_num(x_mean, nan=0.0)
    return x_absmean
...
def meanabs_statistics(x, mask, eps=1e-6, dim=-1):
    x_means = masked_mean(x=x, mask=mask, dim=dim)
    x_means = x_means + eps
    x_shift = torch.zeros_like(x_means)
    
    return x_shift, x_means

def meanabs_scaler(x, x_shift, x_scale):
    return x / x_scale
    
def inv_meanabs_scaler(z, x_shift, x_scale):
    return z * x_scale
