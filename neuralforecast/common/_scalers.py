...
# %% auto 0
__all__ = ['masked_median', 'masked_mean', 'minmax_statistics', 'minmax_scaler', 'inv_minmax_scaler',
           'minmax_treatment_statistics', 'minmax_treatment_scaler', 'inv_minmax_treatment_scaler',
           'minmax1_statistics', 'minmax1_scaler', 'inv_minmax1_scaler', 'std_statistics', 'std_scaler',
           'inv_std_scaler', 'robust_statistics', 'robust_scaler', 'inv_robust_scaler', 'invariant_statistics',
           'invariant_scaler', 'inv_invariant_scaler', 'identity_statistics', 'identity_scaler', 'inv_identity_scaler',
           'TemporalNorm']
...
def minmax_statistics(x, mask, eps=1e-6, dim=-1):
    ...
    **Returns:**<br>
    ...
    mask_inf = mask.clone()
    mask_inf[mask_inf == 0] = torch.inf
    mask_inf[mask_inf == 1] = 0
    x_max = torch.max(
        torch.nan_to_num(x - mask_inf, nan=-torch.inf), dim=dim, keepdim=True
    )[0]
    x_min = torch.min(
        torch.nan_to_num(x + mask_inf, nan=torch.inf), dim=dim, keepdim=True
    )[0]
    ...
    # Change to identity for cases with all values masked
    sum_mask = mask * torch.ones(x_max.shape, device=mask.device)
    sum_mask = torch.sum(sum_mask, dim=dim, keepdim=True)
    x_range[sum_mask == 0] = 1.0
    x_min[sum_mask == 0] = 0.0
...
def minmax_treatment_statistics(x, mask, eps=1e-6, dim=-1):
    """MinMax treatment Scaler
    ...
    **Parameters:**<br>
    ...
    def __init__(self, scaler_type="robust", dim=-1, eps=1e-6):
        ...
        compute_statistics = {
            ...
            "minmax_treatment": minmax_treatment_statistics,
        ...
        scalers = {
            ...
            "minmax_treatment": minmax_treatment_scaler,
        ...
        inverse_scalers = {
            ...
            "minmax_treatment": inv_minmax_treatment_scaler,
