...
class BasePatchall(BaseModel):
    ...
    def __init__(
        ...
        **trainer_kwargs,
        ...
        self.input_patch_len = torch.minimum(
                                    torch.tensor(input_patch_len), 
        ...
        self.output_patch_len = torch.minimum(
                                    torch.tensor(output_patch_len), 
                                    torch.tensor(h)
                                    )
        ...
        if start_padding_enabled:
            ...
            self.padder_train = nn.ConstantPad1d(padding=(0, self.output_patch_len), value=0)
    ...
    def _create_windows(self, batch, step, window_size, w_idxs=None):
        ...
        if step == "train":
            ...
            if (
                ...
                padder_right = nn.ConstantPad1d(padding=(0, self.output_patch_len), value=0) 
    ...
    def training_step(self, batch, batch_idx):
        ...
        window_size = self.input_size + self.output_patch_len
        windows = self._create_windows(batch, window_size=window_size, step="train")
        y_idx = batch["y_idx"]
        #original_outsample_y = torch.clone(windows["temporal"][:, -self.h :, y_idx])
        original_outsample_y = torch.clone(windows["temporal"][:, -self.output_patch_len :, y_idx])
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # using window_size = self.input_size + self.patch_len is given error when identifying windows with w_idxs
        ...
        n_repeats = int(torch.ceil(torch.tensor([self.h/self.output_patch_len])))
        ...
        valid_losses = []
        batch_sizes = []
        previous_preds = torch.zeros(windows_batch_size, self.output_patch_len*n_repeats, 
        ...
        for fi, i in enumerate(repeated_idxs):
            ...
            pos = fi % n_repeats
            if pos != 0:
                seq_preds = previous_preds[:, : self.output_patch_len*pos].clone()
            ...
            previous_preds[:, self.output_patch_len*pos : self.output_patch_len*pos+self.output_patch_len] = output_batch
    ...
    def predict_step(self, batch, batch_idx):
       ...
       # TODO: Hack to compute number of windows
        ...
        previous_preds = torch.zeros(windows_batch_size, 
                                     self.output_patch_len*n_repeats, 
                                     device=windows["temporal"].device
                                    )
